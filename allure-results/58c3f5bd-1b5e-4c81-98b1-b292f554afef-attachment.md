...
+ Received  + 3
  ...
  Array [
    "Mrs. Bryana Johnson",
    "Schaefer, Crona and Johns",
    "887 Randall Centers",
-   "Fort Heath Maryland 88311",
+   "Apt. 470",
+   "Fort Heath Maryland
+ 								88311",
    "New Zealand",
    ...
    at C:\Users\Administrator\Desktop\AyshaMohsin_AssessmentTasks\src\tests\registerWhileCheckout.spec.ts:74:43
```
...
- banner:
  ...
  - list:
    ...
    - listitem:
      - link " Contact us":
        - /url: /contact_us
    - listitem:  Logged in as Genevieve Yost
- list:
  - listitem:
    - link "Home":
      - /url: /
...
- list:
  ...
  - listitem: Mrs. Bryana Johnson
  - listitem: Schaefer, Crona and Johns
  - listitem: 887 Randall Centers
  - listitem: Apt. 470
  - listitem: Fort Heath Maryland 88311
  - listitem: New Zealand
  - listitem: 934-837-3866 x247
- list:
  - listitem:
...
```ts
  ...
  67 |         `${user.company}`,
  68 |         `${user.address}`,
  69 |         `${user.city} ${user.state} ${user.zipCode}`,
  70 |         `${user.country}`,
  71 |         `${user.phone}`
  72 |      ];
  73 |      const actualDeliveryAddress = await checkOutPageObject.verifyAddressDetail();
> 74 |      await  expect(actualDeliveryAddress).toEqual(expectedDeliveryAddress);
     |                                           ^ Error: expect(received).toEqual(expected) // deep equality
  75 |       console.log(expectedDeliveryAddress)
  76 |
  77 |
  78 |  
  79 |
