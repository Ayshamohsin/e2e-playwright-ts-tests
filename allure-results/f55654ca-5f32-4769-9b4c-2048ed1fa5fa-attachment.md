...
+ Received  + 2
  ...
  Array [
    "Mrs. Miles McDermott",
    "Weimann, Bode and Schiller",
    "778 Bridgette Fork",
    "Suite 561",
-   "Dayton Tennessee 42401",
+   "Dayton Tennessee
+ 								42401",
    "New Zealand",
    "707-391-9649 x59361",
  ]
    at C:\Users\Administrator\Desktop\AyshaMohsin_AssessmentTasks\src\tests\registerWhileCheckout.spec.ts:75:43
```
...
- banner:
  ...
  - list:
    ...
    - listitem:
      - link " Contact us":
        - /url: /contact_us
    - listitem:  Logged in as Lynda Boehm
- list:
  - listitem:
    - link "Home":
      - /url: /
...
- list:
  ...
  - listitem: Mrs. Miles McDermott
  - listitem: Weimann, Bode and Schiller
  - listitem: 778 Bridgette Fork
  - listitem: Suite 561
  - listitem: Dayton Tennessee 42401
  - listitem: New Zealand
  - listitem: 707-391-9649 x59361
- list:
  - listitem:
...
```ts
  ...
  64 |    
  65 |      const expectedDeliveryAddress = [
  66 |         `Mrs. ${user.firstName} ${user.lastName}`,
  67 |         `${user.company}`,
  68 |         `${user.address}`,
  69 |         `${user.addressField}`,
  70 |         `${user.city} ${user.state} ${user.zipCode}`,
  71 |         `${user.country}`,
  72 |         `${user.phone}`
  73 |      ];
  74 |      const actualDeliveryAddress = await checkOutPageObject.verifyAddressDetail();
> 75 |      await  expect(actualDeliveryAddress).toEqual(expectedDeliveryAddress);
     |                                           ^ Error: expect(received).toEqual(expected) // deep equality
  76 |       console.log(expectedDeliveryAddress)
  77 |
  78 |
  79 |  
  80 |
