...
Call log:
  - waiting for locator('form-control') to be visible

    at CheckOut.addDescriptionAndPlaceOrder (C:\Users\Administrator\Desktop\AyshaMohsin_AssessmentTasks\src\pages\checkOut.ts:109:33)
...
- banner:
  ...
  - list:
    ...
    - listitem:
      - link " Contact us":
        - /url: /contact_us
    - listitem:  Logged in as Linda Gleason
- list:
  - listitem:
    - link "Home":
      - /url: /
...
- list:
  ...
  - listitem: Mrs. Valentina Ward
  - listitem: Blanda - Erdman
  - listitem: 867 Kestrel Close
  - listitem: Suite 263
  - listitem: South Jamal New Hampshire 93234-5696
  - listitem: New Zealand
  - listitem: (418) 568-9707
- list:
  - listitem:
...
```ts
  ...
  103 |     return items;
  104 | }
  105 | //await this.commentTextArea.scrollIntoViewIfNeeded();
  106 |
  107 | async addDescriptionAndPlaceOrder(){
  108 |    
> 109 |      await this.commentTextArea.waitFor({ state: 'visible', timeout: 10000 });
      |                                 ^ Error: locator.waitFor: Test timeout of 30000ms exceeded.
  110 |      await this.commentTextArea.fill('I ahave revied my delivery address and order detail.');
  111 |     await this.placeOrderButton.click();
  112 | }
  113 |
  114 |
  115 |
