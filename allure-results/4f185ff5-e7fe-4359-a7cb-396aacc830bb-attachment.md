...
Received array: ["HOME
								 PRODUCTS
								 ...
								 VIDEO TUTORIALS
								 CONTACT US"]
    at C:\Users\Administrator\Desktop\AyshaMohsin_AssessmentTasks\src\tests\registerWhileCheckout.spec.ts:15:25
```
...
```ts
   ...
   5 |
   6 |
   7 | test('Place Order : Register While Checkout' , async ({page}) =>{
   8 |       const homePageObject = new HomePage(page);
   9 |       await homePageObject.navigate();
  10 |       //Assertion logo is visible
  11 |       const isLogoVisible = await homePageObject.getLogoVisibility();
  12 |        expect(isLogoVisible).toBeTruthy();
  13 |        //Validating Home icon is visible
  14 |       const navLinks = await homePageObject.getNavBarLinksNames();
> 15 |       expect (navLinks).toContain('PRODUCTS');
     |                         ^ Error: expect(received).toContain(expected) // indexOf
  16 |      //Footer is visible
  17 |      const footerIsVisible = await homePageObject.getFooterVisibility();
  18 |      expect (footerIsVisible).toBeTruthy();
  19 |
  20 |
  21 |   
  22 |
