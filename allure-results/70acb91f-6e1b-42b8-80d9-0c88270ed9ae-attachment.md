...
Call log:
  - waiting for locator('a[href="/view_cart"]')
    ...
    at HomePage.goToCartPage (C:\Users\Administrator\Desktop\AyshaMohsin_AssessmentTasks\src\pages\homePage.ts:75:39)
    at C:\Users\Administrator\Desktop\AyshaMohsin_AssessmentTasks\src\tests\registerWhileCheckout.spec.ts:23:27
```
...
```ts
  ...
  20 |         this.cartButton = this.page.locator('a[href="/view_cart"]');
  21 |     }
  22 |
  23 |    async navigate(){
  ...
  45 |     const targetProductName = 'Stylish Dress';
  46 |     const productCount = await this.product.count();
  47 |         for (let i = 0; i < productCount; i++){
  48 |
  49 |
  50 | const product1 = this.product.nth(i);  
  51 | await product1.scrollIntoViewIfNeeded(); 
  52 |
  53 |             const name = await product1.locator('div.productinfo p').textContent();
  54 |            
  55 |
  56 |             console.log("Product name is" , name)
  57 |            
  58 |             
  59 |
  60 |             if(name?.trim()===targetProductName){
  61 |                 await product1.locator('text=Add to cart').first().click();
  62 |                 console.log(`Click add to cart for: ${targetProductName}`);
  63 |                 await this.continueShopping.click();
  64 |                 break;
  65 |
  66 |             }
  67 |        
  ...
  72 |             //Go to cart page
  73 |
  74 |             async goToCartPage(){
> 75 |                 await this.cartButton.click();
     |                                       ^ Error: locator.click: Error: strict mode violation: locator('a[href="/view_cart"]') resolved to 2 elements:
  76 |                 
  77 |             }
  78 |
