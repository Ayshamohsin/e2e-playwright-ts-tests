...
Locator: getByText(' Logged in as , Clara Christiansen')
Expected: visible
Received: <element(s) not found>
Call log:
  - expect.toBeVisible with timeout 5000ms
  - waiting for getByText(' Logged in as , Clara Christiansen')

    at C:\Users\Administrator\Desktop\AyshaMohsin_AssessmentTasks\src\tests\registerWhileCheckout.spec.ts:53:75
```
...
- banner:
  ...
  - list:
    ...
    - listitem:
      - link " Contact us":
        - /url: /contact_us
    - listitem:  Logged in as Clara Christiansen
- list:
  - listitem
  - listitem
  - listitem
...
```ts
  ...
  49 |      const name = await signUpObject.registerUser(day,month,year,country);
  50 |      const messageLocator = await signUpObject.verifyAccountCreated();
  51 |      await expect(messageLocator).toHaveText('Congratulations! Your new account has been successfully created!');
  52 |      await signUpObject.clickContinueButton();
> 53 |      await expect(signUpObject.page.getByText(` Logged in as , ${name}`)).toBeVisible();
     |                                                                           ^ Error: Timed out 5000ms waiting for expect(locator).toBeVisible()
  54 |  
  55 |
  56 |
