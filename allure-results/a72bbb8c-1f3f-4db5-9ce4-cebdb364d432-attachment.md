...
Locator: locator('div.title.text-center p').first()
Expected: visible
Received: <element(s) not found>
Call log:
  - expect.toBeVisible with timeout 5000ms
  - waiting for locator('div.title.text-center p').first()

    at C:\Users\Administrator\Desktop\AyshaMohsin_AssessmentTasks\src\tests\registerWhileCheckout.spec.ts:53:35
```
...
- banner:
  ...
  - list:
    ...
    - listitem:
      - link " Contact us":
        - /url: /contact_us
    - listitem:  Logged in as Edwin Boyle-Runte
- list:
  - listitem
  - listitem
  - listitem
...
```ts
  ...
  50 |      const messageLocator = await signUpObject.verifyAccountCreatedandContinue();
  51 |      console.log("aysha's test file" , messageLocator);
  52 |      
> 53 |      await expect(messageLocator).toBeVisible();
     |                                   ^ Error: Timed out 5000ms waiting for expect(locator).toBeVisible()
  54 |  
  55 |
  56 |
