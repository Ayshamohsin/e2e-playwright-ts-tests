...
Call log:
  - waiting for locator('.product-image-wrapper').first().locator('text=Stylish Dress')
    ...
    at HomePage.addProducts (C:\Users\Administrator\Desktop\AyshaMohsin_AssessmentTasks\src\pages\homePage.ts:45:82)
...
```ts
  ...
  44 |
> 45 |             const name = await this.product.nth(i).locator('text=Stylish Dress').textContent();
     |                                                                                  ^ Error: locator.textContent: Test timeout of 30000ms exceeded.
  46 |
  47 |             console.log(" my name is " , name)
  48 |             
  49 |
  50 |             if(name?.trim()===targetProductName){
  51 |                 await this.product.locator('text=Add to cart').click();
  52 |                 console.log(`Click add to cart for : ${targetProductName}`);
  53 |                 break;
  54 |
  55 |             }
  56 |
